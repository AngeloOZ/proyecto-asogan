...
export const MainMartillador = ({ lote, ultimaPuja, evento }: Props) => {
  const { data } = useSWR(`/subastas/pujas?lote=${lote?.id_lote}`, fetcher, {
    refreshInterval: 1500,
  }) as { data: PujasRequest };
          ...
          >
            <Box
              style={{
                textAlign: "center",
                padding: 5,
                borderBottom: "1px #dad8db dashed",
              }}
            >
              <Typography
                component="h3"
                ...
                <Stack spacing={0.5}>
                  {data.mejoresPujas.map((puja) => (
                    <Item key={puja.id_puja} sx={{ fontSize: "11px" }}>
                      ...
                      <strong>USUARIO:</strong> {puja?.usuario?.nombres}
                      <br />
                      <strong>VALOR:</strong>{" "}
                      {"$" + parseFloat(puja.puja).toFixed(2)}
                    </Item>
                  ))}
                </Stack>
              ) : (
